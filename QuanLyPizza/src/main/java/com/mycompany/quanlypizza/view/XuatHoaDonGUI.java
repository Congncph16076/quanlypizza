...
public class XuatHoaDonGUI extends JDialog {
    ...
    private DlgTimKhachView timKhachUI = new DlgTimKhachView();
    private DlgTimMaGiamGiaView timMaUI = new DlgTimMaGiamGiaView();
    ...
    private void xuLyHienThiHoaDon() {
        ...
        if (timMaUI.maGiamTimDuoc != null) {
            ...
            if (tongTien >= timMaUI.maGiamTimDuoc.getDieuKien()) {
                ...
                new MyDialogCommon("Không đủ điều kiện nhận ưu đãi!", MyDialogCommon.ERROR_DIALOG);
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (txtTenKhach.getText().equals("")) {
            new MyDialogCommon("Xin chọn khách hàng", MyDialogCommon.ERROR_DIALOG);
            return;
        }
        if (txtMaGiam.getText().equals("")) {
            new MyDialogCommon("Xin chọn mã giảm", MyDialogCommon.ERROR_DIALOG);
            return;
        }
        xuLyHienThiHoaDon();
        btnInHoaDon.setEnabled(true);

        hoaDonServiceImp.addNew(DlgTimKhachView.khachHangTimDuoc.getMaKH(), nhanVien, tongTien, "Đã thanh toán");
        ...
        for (Vector vec : dsGioHang) {
            ...
            cTHoaDonServiceImp.addNew(maSP, soLuong, donGia, thanhTien);
        }
        btnThanhToan.setEnabled(false);
